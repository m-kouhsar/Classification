...
for(i in 1:length(models)){
  ...
  tryCatch(expr = {
    ...
    for(j in 1:length(folds)){
      train.roc.obj[[j]] <- roc(response=fit.pred$obs[fit.pred$Resample==folds[j]],predictor=fit.pred[,levels(data.train$Class)[1]][fit.pred$Resample==folds[j]])
  ...
  error=function(e){
    print(e)
      ...
      # This runs ALWAYS (success or failure) immediately after the block finishes
      if(Do.Parallel) {
